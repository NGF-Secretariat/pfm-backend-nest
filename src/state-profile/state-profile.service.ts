...
export class StateProfileService {
  ...
  async getProfileBySlug(slug: string) {
    ...
    const revItems = items.filter(
      (i) => i.description === 'Total Revenue' || i.description === 'Revenue',
    );
    const fallbackRevItem = items.find(
      (i) => i.description === 'Total Revenue (including Opening Balance)',
    );
    const expItems = items.filter(
      (i) =>
        i.description === 'TOTAL EXPENDITURE' ||
        i.description === 'EXPENDITURE',
    );
    const fallbackExpItem = items.find(
      (i) => i.description === 'Total Expenditure',
    );
    const capItems = items.filter(
      (i) =>
        i.description === 'Capital Expenditure' ||
        i.description === 'Total Capital Expenditure',
    );
    const personnelItems = items.filter(
      (i) =>
        i.description === 'Personnel Expenditure' ||
        i.description === 'Personnel',
    );
    const otherRecItems = items.filter(
      (i) =>
        i.description === 'Other Recurrent Expenditure' ||
        i.description === 'Other Recurrent Cost',
    );
    const totalRecItem = items.find(
      (i) => i.description === 'Total Recurrent Expenditure',
    );
    ...
    for (const year of years) {
      ...
      if (revItems.length > 0) {
        actRev =
          actYear
            .find((a) => revItems.some((i) => i.id === a.itemId))
            ?.amount.toNumber() || 0;
      }
      if (actRev === 0 && fallbackRevItem) {
        actRev =
          actYear
            .find((a) => a.itemId === fallbackRevItem.id)
            ?.amount.toNumber() || 0;
      }
      ...
      if (revItems.length > 0) {
        budRev =
          budYear
            .find((b) => revItems.some((i) => i.id === b.itemId))
            ?.amount.toNumber() || 0;
      }
      if (budRev === 0 && fallbackRevItem) {
        budRev =
          budYear
            .find((b) => b.itemId === fallbackRevItem.id)
            ?.amount.toNumber() || 0;
      }
      ...
      if (expItems.length > 0) {
        actExp =
          actYear
            .find((a) => expItems.some((i) => i.id === a.itemId))
            ?.amount.toNumber() || 0;
      }
      if (actExp === 0 && fallbackExpItem) {
        actExp =
          actYear
            .find((a) => a.itemId === fallbackExpItem.id)
            ?.amount.toNumber() || 0;
      }
      ...
      if (expItems.length > 0) {
        budExp =
          budYear
            .find((b) => expItems.some((i) => i.id === b.itemId))
            ?.amount.toNumber() || 0;
      }
      if (budExp === 0 && fallbackExpItem) {
        budExp =
          budYear
            .find((b) => b.itemId === fallbackExpItem.id)
            ?.amount.toNumber() || 0;
      }
      ...
      if (actRecVal === 0 && totalRecItem) {
        actRecVal =
          actYear
            .find((a) => a.itemId === totalRecItem.id)
            ?.amount.toNumber() || 0;
      }
      ...
      if (budRecVal === 0 && totalRecItem) {
        budRecVal =
          budYear
            .find((b) => b.itemId === totalRecItem.id)
            ?.amount.toNumber() || 0;
      }
  ...
  async updateProfileBySlug(slug: string, updateData: any) {
    const existing = await this.prisma.stateProfile.findUnique({
      where: { slug },
    });
